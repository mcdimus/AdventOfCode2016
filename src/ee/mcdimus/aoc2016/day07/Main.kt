package ee.mcdimus.aoc2016.day07
 ...
 * @see <a href="http://adventofcode.com/2016/day/7">Day 7: Internet Protocol Version 7</a>
 */
class Main {
  ...
  companion object {
    ...
    fun main(vararg args: String) {
      ...
      val ips = input.map(::IP)

      println("IPs that support TLS: ${ips.count(IP::supportsTLS)}")
      println("IPs that support SSL: ${ips.count(IP::supportsSSL)}")
    }
  }

}

class IP(val value: String) {

  fun supportsTLS(): Boolean {
    return Regex("((\\w)(\\w)(?!\\2)\\3\\2)(?=[^\\[]*])").find(value) == null
        && Regex("((\\w)(\\w)(?!\\2)\\3\\2)(?![^\\[]*\\])").find(value) != null
  }

  fun supportsSSL(): Boolean {
    return Regex(".*?((\\w)(?!\\2)(.)\\2)(?![^\\[]*]).*?(\\3\\2\\3)(?=[^\\[]*])").find(value) != null
        || Regex(".*?((\\w)(?!\\2)(.)\\2)(?=[^\\[]*]).*?(\\3\\2\\3)(?![^\\[]*])").find(value) != null
  }
